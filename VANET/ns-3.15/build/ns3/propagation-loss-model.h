...
class ShadowingPropagationLossModel : public PropagationLossModel
{
public:
  static TypeId GetTypeId (void);
  
  ShadowingPropagationLossModel ();

  // Parameters are all accessible via attributes.

private:
  ShadowingPropagationLossModel (const ShadowingPropagationLossModel& o);
  ShadowingPropagationLossModel& operator= (const ShadowingPropagationLossModel& o);
  ...
  Ptr<NormalRandomVariable> m_normalRvM1;
};
...
class TwoRandShadowingLineOfSightPropagationLossModel : public PropagationLossModel
{
public:
  static TypeId GetTypeId (void);
  
  TwoRandShadowingLineOfSightPropagationLossModel ();
  
    // Parameters are all accessible via attributes.

private:
  TwoRandShadowingLineOfSightPropagationLossModel (const TwoRandShadowingLineOfSightPropagationLossModel& o);
  TwoRandShadowingLineOfSightPropagationLossModel& operator= (const TwoRandShadowingLineOfSightPropagationLossModel& o);
  ...
  double m_distance0 ; // 1st breakpoint distance d0
  double m_distanceb ; // Breakpoint distance db
  double m_pathLoss0 ; // Free space path loss plus the accumulative antenna gain (PLf + Ga)
  double m_exponentN1 ; // Path loss exponent until the breakpoint distance b
  double m_sigmaM1 ; // Standard deviation until the breakpoint distance b
  double m_exponentN2 ; // Path loss expoent after breakpoint distance b
  double m_sigmaM2 ; // Standard deviation after the breakpoint distance b

  Ptr<NormalRandomVariable> m_normalRvM1;
  Ptr<NormalRandomVariable> m_normalRvM2;
};
