...
private:
  ...
  // Trace to signal what is going on at StartReceive
  TracedCallback<Ptr<const Packet> > m_startRxErrorTrace;
  // Trace to signal that packet arrived with energy below the energy detection threshold
  TracedCallback<Ptr<const Packet> > m_energyDetectionFailed;
  // Trace  to signal a success at the end of the reception
  TracedCallback<Ptr<const Packet> > m_rxOkTrace;
  // Trace to signal a failure at the end of the reception
