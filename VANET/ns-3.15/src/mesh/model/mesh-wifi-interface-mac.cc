...
// Added for making compatibility with PhySimWifiPhy
...
MeshWifiInterfaceMac::GetFrequencyChannel () const
{
  ...
  if (phy != 0)
    ...
    Ptr<PhySimWifiPhy> phy = m_phy->GetObject<PhySimWifiPhy> ();
    if (phy != 0)
      {
        return phy->GetChannelNumber ();
      }
...
MeshWifiInterfaceMac::SwitchFrequencyChannel (uint16_t new_id)
{
   ...
   */
  // Channel switching in case of YansWifi usage
  bool configured = false;
  Ptr<YansWifiPhy> yansPhy = m_phy->GetObject<YansWifiPhy> ();
  if (yansPhy) {
    yansPhy->SetChannelNumber (new_id);
    configured = true; // BH 05/11/2013
  }
  
  // This is done despite channel switching is TODO in PhySimWifiPhy, BH 16/07/2013
  Ptr<PhySimWifiPhy> physimPhy = m_phy->GetObject<PhySimWifiPhy> ();
  if (physimPhy) {
    physimPhy->SetChannelNumber (new_id);
    configured = true; // BH 05/11/2013
  }

  if (!configured) // BH 05/11/2013
    NS_FATAL_ERROR ("No phy interface in MeshWifiInterfaceMac configuration");
