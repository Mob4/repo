...
NS_OBJECT_ENSURE_REGISTERED (ShadowingPropagationLossModel);

TypeId
ShadowingPropagationLossModel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ShadowingPropagationLossModel")
    .SetParent<PropagationLossModel> ()
    .AddConstructor<ShadowingPropagationLossModel> ()
    .AddAttribute ("StandardDeviation",
                   "The standard deviation of the shadow fading (dB)",
                   DoubleValue (2.0),
                   MakeDoubleAccessor (&ShadowingPropagationLossModel::m_stdDeviationDb),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("MinimumDistance",
                   ...
                   DoubleValue (50.0),
                   MakeDoubleAccessor (&ShadowingPropagationLossModel::m_minDistance),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("NormalRv",
                   "Access to the underlying NormalRandomVariable",
                   StringValue ("ns3::NormalRandomVariable"),
                   MakePointerAccessor (&ShadowingPropagationLossModel::m_normalRvM1),
                   MakePointerChecker<NormalRandomVariable> ())  ;
  return tid;

}

ShadowingPropagationLossModel::ShadowingPropagationLossModel ()
{
}

double 
ShadowingPropagationLossModel::DoCalcRxPower (double txPowerDbm,
                                                     Ptr<MobilityModel> a,
                                                     Ptr<MobilityModel> b) const
{
  ...
  double pathLoss = m_normalRvM1->GetValue (0, (m_stdDeviationDb * m_stdDeviationDb));

  NS_LOG_DEBUG ("ShadowingPropagationLossModel::DoCalcRxPower() m_pathLoss = " << pathLoss);

  return txPowerDbm - pathLoss;
}

int64_t
ShadowingPropagationLossModel::DoAssignStreams (int64_t stream)
{
...
NS_OBJECT_ENSURE_REGISTERED (TwoRandShadowingLineOfSightPropagationLossModel);

TypeId
TwoRandShadowingLineOfSightPropagationLossModel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::TwoRandShadowingLineOfSightPropagationLossModel")
    .SetParent<PropagationLossModel> ()
    .AddConstructor<TwoRandShadowingLineOfSightPropagationLossModel> ()
    .AddAttribute ("Distance0",
                   "1st breakpoint distance d0.",
                   DoubleValue (10.0),
                   MakeDoubleAccessor (&TwoRandShadowingLineOfSightPropagationLossModel::m_distance0),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("Distanceb",
                   "breakpoint distance",
                   DoubleValue (104), // this value is given based on the reference paper 
                   MakeDoubleAccessor (&TwoRandShadowingLineOfSightPropagationLossModel::m_distanceb),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("PathLoss0",
                   "Free space path loss plus the accumulative antenna gain (PLf + Ga)",
                   DoubleValue (-56.5),
                   MakeDoubleAccessor (&TwoRandShadowingLineOfSightPropagationLossModel::m_pathLoss0),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("ExponentN1",
                   "Path loss exponent until the breakpoint distance b",
                   DoubleValue (-1.81),
                   MakeDoubleAccessor (&TwoRandShadowingLineOfSightPropagationLossModel::m_exponentN1),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("ExponentN2",
                   "Path loss exponent after the breakpoint distance b",
                   DoubleValue (-2.85),
                   MakeDoubleAccessor (&TwoRandShadowingLineOfSightPropagationLossModel::m_exponentN2),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("SigmaM1",
                   "Standard deviation until the breakpoint distance b",
                   DoubleValue (4.15),
                   MakeDoubleAccessor (&TwoRandShadowingLineOfSightPropagationLossModel::m_sigmaM1),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("SigmaM2",
                   "Standard deviation after the breakpoint distance b",
                   DoubleValue (4.15),
                   MakeDoubleAccessor (&TwoRandShadowingLineOfSightPropagationLossModel::m_sigmaM2),
                   MakeDoubleChecker<double> ())
     .AddAttribute ("NormalRvM1",
                   "Access to the underlying NormalRandomVariable for M1",
                   StringValue ("ns3::NormalRandomVariable"),
                   MakePointerAccessor (&TwoRandShadowingLineOfSightPropagationLossModel::m_normalRvM1),
                   MakePointerChecker<NormalRandomVariable> ())
     .AddAttribute ("NormalRvM2",
                   "Access to the underlying NormalRandomVariable for M2",
                   StringValue ("ns3::NormalRandomVariable"),
                   MakePointerAccessor (&TwoRandShadowingLineOfSightPropagationLossModel::m_normalRvM2),
                   MakePointerChecker<NormalRandomVariable> ())
  ;
  return tid;
...
TwoRandShadowingLineOfSightPropagationLossModel::TwoRandShadowingLineOfSightPropagationLossModel ()
{
}

double 
TwoRandShadowingLineOfSightPropagationLossModel::DoCalcRxPower (double txPowerDbm,
                                                     Ptr<MobilityModel> a,
                                                     Ptr<MobilityModel> b) const
{
  double distance = a->GetDistanceFrom (b);
  ...
  if (distance < m_distance0) {
      ...
      pathLoss = m_pathLoss0 + 10 * m_exponentN1 * log10 (distance / m_distance0)
        + m_normalRvM1->GetValue (0, (m_sigmaM1 * m_sigmaM1));
  }
  else
    if (distance < m_distanceb) {
      ...
      pathLoss = m_pathLoss0 + 10 * m_exponentN1 * log10 (m_distanceb / m_distance0) 
        ...
        + m_normalRvM2->GetValue (0, (m_sigmaM2 * m_sigmaM2)) ;
    }

  NS_LOG_DEBUG ("TwoRandShadowingLineOfSightPropagationLossModel::DoCalcRxPower() pathLoss = " << pathLoss);
