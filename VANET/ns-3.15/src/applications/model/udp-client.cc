...
namespace ns3 {
  ...
  UdpClient::Send (void)
  {
    ...
    if (Ipv4Address::IsMatchingType (m_peerAddress))
      {
        std::cout << "--------------> FD: UdpClient:: packet " << p->GetUid() << " sent at " << Simulator::Now ().GetSeconds () << "s: " << (m_size-(8+4)) << " bytes to IPV4 " << Ipv4Address::ConvertFrom (m_peerAddress) << std::endl ;
  ...
  /** Access to the number of messages sent
   */
  uint32_t UdpClient::GetMessagesSent() {
    return m_sent ;
  }
