...
YansWifiPhy::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::YansWifiPhy")
    ...
    .AddTraceSource ("StartRxError", "First signal of a packet has arrived at the receiving node but it could not be considered because transceiver was in SYNC, TX or RX state",
    		           MakeTraceSourceAccessor (&YansWifiPhy::m_startRxErrorTrace))
    .AddTraceSource ("EnergyDetectionFailed", "First signal of a packet has arrived at the receiving node but the signal strength is below energy detection threshold",
    							 MakeTraceSourceAccessor (&YansWifiPhy::m_energyDetectionFailed))
    .AddTraceSource ("RxOk",
                                 ...
                                 enum WifiPreamble preamble)
{
  ...
  switch (m_state->GetState ())
    ...
    case YansWifiPhy::SWITCHING:
      ...
      NotifyRxDrop (packet); m_startRxErrorTrace (packet);
      /*
    ...
    case YansWifiPhy::IDLE:
      if (rxPowerW > m_edThresholdW)
      ...
      else
        {
          ...
          NotifyRxDrop (packet); m_energyDetectionFailed (packet);
          goto maybeCcaBusy;
        }
      break;
    }
...
YansWifiPhy::EndReceive (Ptr<Packet> packet, Ptr<InterferenceHelper::Event> event)
{
  ...
  NS_LOG_DEBUG ("mode=" << (event->GetPayloadMode ().GetDataRate ()) <<
	...
	outPut << "YansWifiPhy::EndReceive: -AtPhy " << Simulator::Now().GetSeconds()
  ...
  if (m_random->GetValue () > snrPer.per)
    {
      NotifyRxEnd (packet); m_rxOkTrace (packet);
    ...
    {
      /* failure. */
      NotifyRxDrop (packet); m_rxErrorTrace (packet);
