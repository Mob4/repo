...
/**
 * \brief A model based on "Measurement Based Shadow fading model for VANETs. Implmentation of LOS model based on two points 
 * ABBAS implementation see paper for more information
 */
class PhySimTwoRandShadowingLineOfSightPropagationLoss : public PhySimPropagationLossModel
{
public:
  ...
  //PhySimTwoRandShadowingLineOfSightPropagationLoss (enum ns3::Scenario);
  virtual ~PhySimTwoRandShadowingLineOfSightPropagationLoss ();
  
  double GetDistance0() const { return m_distance0; }
  void SetDistance0(double distance0) { this->m_distance0 = distance0; }
  double GetDistanceb() const { return m_distanceb; }
  void SetDistanceb(double distanceb) { this->m_distanceb = distanceb; }
  double GetExponentn1() const { return m_exponentn1; }
  void SetExponentn1(double exponentn1) { this->m_exponentn1 = exponentn1; }
  double GetExponentn2() const { return m_exponentn2; }
  void SetExponentn2(double exponentn2) { this->m_exponentn2 = exponentn2; }
  double GetPathLoss0() const { return m_pathLoss0; }
  void SetPathLoss0(double pathLoss0) { this->m_pathLoss0 = pathLoss0; }
  double GetSigma1() const { return m_sigma1; }
  void SetSigma1(double sigma1) { this->m_sigma1 = sigma1; if (m_normalVariableSigma1) delete m_normalVariableSigma1 ; m_normalVariableSigma1 = new NormalVariable (0, (m_sigma1 * m_sigma1)); }
  double GetSigma2() const { return m_sigma2; }
  void SetSigma2(double sigma2) { this->m_sigma2 = sigma2; if (m_normalVariableSigma2) delete m_normalVariableSigma2 ; m_normalVariableSigma2 = new NormalVariable (0, (m_sigma2 * m_sigma2)); }
  
private: 
  PhySimTwoRandShadowingLineOfSightPropagationLoss (const PhySimTwoRandShadowingLineOfSightPropagationLoss &o);
  PhySimTwoRandShadowingLineOfSightPropagationLoss &operator = (const PhySimTwoRandShadowingLineOfSightPropagationLoss &o);
  virtual void DoCalcRxPower (Ptr<PhySimWifiPhyTag> tag, Ptr<MobilityModel> a, Ptr<MobilityModel> b) const;

  // 1st breakpoint distance d0
  double m_distance0 ;
  // Breakpoint distance db
  double m_distanceb ; 
  ...
  double m_sigma1 ;
  // Path loss expoent after breakpoint distance b
  double m_exponentn2 ;
  // Standard deviation after the breakpoint distance b
  double m_sigma2 ;


  NormalVariable *m_normalVariableSigma1;
  NormalVariable *m_normalVariableSigma2;
...
public:
  ...
  //void SetChannelProfile (itpp::Channel_Specification profile,bool user);
  enum itpp::CHANNEL_PROFILE GetChannelProfile () const;
  //itpp::Channel_Specification GetChannelProfile () const;
...
private:
  ...
  itpp::Channel_Specification m_user_channelProfile;
  bool m_user_flag;
