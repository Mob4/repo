  ...
  for (int32_t i = 0; i < cumNoise.size () - 80; i += 80)
    {
      ...
      //HB power value corrected to be in dBm
      if (PhySimHelper::WToDbm (strength) < energydBm)
      {
  ...
  while (it != m_noiseChunks.end ())
    {
      ...
      // and then we check for the 2nd case
      else if (indexStart >= nextIndex && indexEnd <= lastIndex)
        {
          ...
          if (offset > 0)
            {
        	  //HB noise value corrected to be in dBm
        	  itpp::cvec noise = sqrt ( pow (10, ((m_noiseFloorDbm -30.0) / 10)) ) * itpp::randn_c (offset);
  ...
  if (nextIndex < lastIndex)
    {
      uint32_t numSamples = (lastIndex - nextIndex);
      //HB noise value corrected to be in dBm
      itpp::cvec noise = sqrt ( pow (10, ((m_noiseFloorDbm -30.0) / 10)) ) * itpp::randn_c (numSamples);
...
PhySimInterferenceHelper::CalculatePayloadSinr (Ptr<PhySimInterferenceHelper::Event> event)
{
  ...
  /*BH*/ //itpp::cvec payload = rxSamples.get (400, rxSamples.size () - 1);
  itpp::cvec payload = rxSamples.get (400, rxSamples.size () - 1 );
  ...
  // /*BH*/ std::cout << "CalculatePayloadSinr: Payload size: " << payload.size() << " cumnoise size: " << cumNoise.size();
  cumNoise -= payload;
...
PhySimInterferenceHelper::CalculateOverallSinr (Ptr<PhySimInterferenceHelper::Event> event)
{
  NS_LOG_FUNCTION (event);
  itpp::cvec rxSamples = event->GetWifiPhyTag ()->GetRxedSamples ();
  /*BH*/ //  itpp::cvec total = rxSamples.get (0, rxSamples.size () - 1);
  itpp::cvec total = rxSamples.get (0, rxSamples.size () - 1 ) ;
  itpp::cvec cumNoise = GetCumulativeSamples (event->GetStartTime (), event->GetEndTime ());
  // /*BH*/ std::cout << "CalculateOverallSinr: Payload size: " << total.size() << " cumnoise size: " << cumNoise.size();
  cumNoise -= total;
