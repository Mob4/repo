...
{
  /*BH*/ std::ostringstream outPut;
  outPut << "  -> rxPower calculation with " << this->GetInstanceTypeId(); /*BH*/

  DoCalcRxPower (tag, a, b);

  /*BH*/ outPut << ": rxPower -> "
           << PhySimHelper::WToDbm (PhySimHelper::GetOFDMSymbolSignalStrength (tag->GetRxedSamples()))
           << " dBm";
  NS_LOG_DEBUG (outPut.str()); /*BH*/
                                                ...
                                                Ptr<MobilityModel> a, Ptr<MobilityModel> b) const
{
  ...
  itpp::cvec input = tag->GetRxedSamples ();
          ...
          //ss1 << packet->GetUid();
          std::string file_name1 = "coeffs_itpp/packet.it";
          itpp::it_file ff;
          ff.open(file_name1);
          ff << itpp::Name("OFDM_TX") << input;
...
NS_OBJECT_ENSURE_REGISTERED (PhySimTwoRandShadowingLineOfSightPropagationLoss);

// Method to build with default urban values
TypeId
PhySimTwoRandShadowingLineOfSightPropagationLoss::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::PhySimTwoRandShadowingLineOfSightPropagationLoss")
    .SetParent<PhySimPropagationLossModel> ()
    .AddConstructor<PhySimTwoRandShadowingLineOfSightPropagationLoss> ()
    .AddAttribute ("Distance0",
                   "1st breakpoint distance d0.",
                   DoubleValue (10.0),
                   MakeDoubleAccessor (&PhySimTwoRandShadowingLineOfSightPropagationLoss::m_distance0),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("Distanceb",
                   "breakpoint distance",
                   DoubleValue (104), // this value is given based on the reference paper 
                   MakeDoubleAccessor (&PhySimTwoRandShadowingLineOfSightPropagationLoss::m_distanceb),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("PathLoss0",
                   "Free space path loss plus the accumulative antenna gain (PLf + Ga)",
                   DoubleValue (-63.9),
                   MakeDoubleAccessor (&PhySimTwoRandShadowingLineOfSightPropagationLoss::m_pathLoss0),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("Exponent1",
                   "Path loss exponent until the breakpoint distance b",
                   DoubleValue (-1.81),
                   MakeDoubleAccessor (&PhySimTwoRandShadowingLineOfSightPropagationLoss::m_exponentn1),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("Exponent2",
                   "Path loss exponent after the breakpoint distance b",
                   DoubleValue (-2.85),
                   MakeDoubleAccessor (&PhySimTwoRandShadowingLineOfSightPropagationLoss::m_exponentn2),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("Sigma1",
                   "Standard deviation until the breakpoint distance b",
                   DoubleValue (4.15),
                   MakeDoubleAccessor (&PhySimTwoRandShadowingLineOfSightPropagationLoss::SetSigma1,
                                       &PhySimTwoRandShadowingLineOfSightPropagationLoss::GetSigma1),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("Sigma2",
                   "Standard deviation after the breakpoint distance b",
                   DoubleValue (4.15),
                   MakeDoubleAccessor (&PhySimTwoRandShadowingLineOfSightPropagationLoss::SetSigma2,
                                       &PhySimTwoRandShadowingLineOfSightPropagationLoss::GetSigma2),
                   MakeDoubleChecker<double> ())
    ;
...
PhySimTwoRandShadowingLineOfSightPropagationLoss::PhySimTwoRandShadowingLineOfSightPropagationLoss ()
  : PhySimPropagationLossModel (),
    m_normalVariableSigma1 (0),
    m_normalVariableSigma2 (0)
{
}
...
void
PhySimTwoRandShadowingLineOfSightPropagationLoss::DoCalcRxPower (Ptr<PhySimWifiPhyTag> tag, Ptr<MobilityModel> a, Ptr<MobilityModel> b) const
{
  ...
  if (distance < m_distance0)
    {
      ...
      // See paper p7 "there are only a few samples available for d < 10 m, thus the validity range of the model is set to d > 10 m and let d0 = 10 m"
      pathLoss = m_pathLoss0 + 10 * m_exponentn1 * log10 (distance / m_distance0) + m_normalVariableSigma1->GetValue ();

    }
  else if (distance < m_distanceb)
    {
	  randValue = m_normalVariableSigma1->GetValue () ;
      pathLoss = m_pathLoss0 + 10 * m_exponentn1 * log10 (distance / m_distance0) 
        + m_normalVariableSigma1->GetValue ();
    }
  else 
    {
	  randValue = m_normalVariableSigma2->GetValue () ;
      pathLoss = m_pathLoss0 + 10 * m_exponentn1 * log10 (m_distanceb / m_distance0) 
        + 10 * m_exponentn2 * log10 (distance / m_distanceb)
        + randValue ;


    }

  //pathLoss = (-1.0) * pathLoss;
  //NS_LOG_UNCOND ("---------------------------------> FD::PhySimTwoRandShadowingLineOfSightPropagationLoss::DoCalcRxPower() pathLoss = " << pathLoss << " with rand value = " << randValue);
  //NS_LOG_UNCOND("FD" << ";" << distance << ";" << pathLoss);

  itpp::cvec input = tag->GetRxedSamples ();
  itpp::cvec output = input * sqrt (pow (10, pathLoss / 10.0)); // passage en Volts
...
PhySimTappedDelayLinePropagationLoss::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::PhySimTappedDelayLinePropagationLoss")
    ...
    /* HB Begin */
    .AddAttribute ("User channel profile",
                   "Flag to set Default is zero", BooleanValue (false),
                    MakeBooleanAccessor (&PhySimTappedDelayLinePropagationLoss::m_user_flag),
                    MakeBooleanChecker ())
    /* HB End */
    .AddAttribute ("ChannelProfile",
                   "The channel profile which used for this TDL model.",
                   EnumValue (itpp::ITU_Vehicular_A),
                   //EnumValue (itpp::Channel_Specification),
  ...
  Ptr<PhySimWifiPhyTag> tag, Ptr<MobilityModel> a, Ptr<MobilityModel> b) const
{
  ...
  /* HB */m_channel.set_channel_profile(m_channelProfile,tag->GetSampleDuration ());
  ...
  /* HB */ output.set_size(input.size(),false);
