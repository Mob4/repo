...
public:
  ...
  RequestQueue (uint32_t maxLen, Time routeToQueueTimeout) :
    m_maxLen (maxLen), m_queueTimeout (routeToQueueTimeout), m_pktDropped (0)
  {
  }
  ...
  uint32_t GetPktDropped () const { return m_pktDropped; }
  //\}
...
private:
  ...
  /// The number of packets that are dropped in a queue
