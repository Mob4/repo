...
#define NS_LOG_APPEND_CONTEXT                                   \
  if (m_ipv4) { std::clog << "[node " << m_ipv4->GetObject<Node> ()->GetId () << "] RoutingProtocol::" << __func__ << ": At " << Simulator::Now().GetSeconds() << " s "; } 
                                      ...
                                      UnicastForwardCallback ucb, ErrorCallback ecb)
{
  ...
  /*BH*/ /*NS_LOG_DEBUG ("Packet " << p->GetUid() << " is in the node queue: " << result);*/
  if (result)
    {
      NS_LOG_LOGIC ("Add packet " << p->GetUid () << " to queue. Protocol " << (uint16_t) header.GetProtocol ());
      RoutingTableEntry rt;
      bool result = m_routingTable.LookupRoute (header.GetDestination (), rt);
      /*BH*/ NS_LOG_DEBUG ("Routing table lookup for " << header.GetDestination () << " is " << result);
      if(!result || ((rt.GetFlag () != IN_SEARCH) && result))
        {
          /*BH*/ NS_LOG_DEBUG ("Send new RREQ for outbound packet to " <<header.GetDestination ());
...
RoutingProtocol::SendRequest (Ipv4Address dst)
{
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j =
         m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      /*BH*/ NS_LOG_DEBUG ("  At "<< Simulator::Now().GetSeconds()
        << "s, AODV node " << iface.GetLocal ()
        << " AODVSendRequest "
        << " (" << packet->GetUid()
        << ") for "<< dst
        );
...
RoutingProtocol::RecvAodv (Ptr<Socket> socket)
{
  ...
  uint32_t localNode = this->GetObject<Node> ()->GetId();
  ClTagTrans clTT;
  if (packet->PeekPacketTag(clTT))
    NS_LOG_DEBUG ("  "  << Simulator::Now().GetSeconds()
      << "s, AODV node " << localNode
      << " received an AODV packet " << packet->GetUid()
      << " of type " <<  tHeader.Get()
      << " from  " << sender
      << " to " << receiver
      << " (" << clTT.GetRxPower()
  ...
  switch (tHeader.Get ())
    ...
    case AODVTYPE_RREQ:
      {
        ...
        NS_LOG_DEBUG ("=========> AODV message from " << sender << " packet " << packet->GetUid () << " with type: " << tHeader.Get () << "RREQ");
    ...
    case AODVTYPE_RERR:
      {
        NS_LOG_DEBUG ("=========> AODV message from " << sender << " packet " << packet->GetUid () << " with type: " << tHeader.Get () << "RERR");
    ...
    case AODVTYPE_RREP_ACK:
      {
        ...
        NS_LOG_DEBUG ("=========> AODV message from " << sender << " packet " << packet->GetUid () << " with type: " << tHeader.Get () <<  "RREP_ACK");
  ...
  if (!m_routingTable.LookupRoute (origin, toOrigin))
    {
      ...
      NS_LOG_UNCOND (" Recv RREQ for " << rreqHeader.GetDst ()
        << " new routing table entry is created"
      );
    }
  else
    {
...
RoutingProtocol::SendReply (RreqHeader const & rreqHeader, RoutingTableEntry const & toOrigin)
{
  ...
  uint32_t localNode = this->GetObject<Node> ()->GetId();
  NS_LOG_DEBUG ( "RoutingProtocol::SendReply At" << Simulator::Now().GetSeconds()
      << "s, AODV destination node " << localNode
      << " send and RREP packet " << packet->GetUid()
      << " for " << rreqHeader.GetDst ()
  );
}
...
RoutingProtocol::SendReplyByIntermediateNode (RoutingTableEntry & toDst, RoutingTableEntry & toOrigin, bool gratRep)
{
  ...
  uint32_t localNode = this->GetObject<Node> ()->GetId();
  NS_LOG_DEBUG ( "RoutingProtocol::SendReplyByIntermediateNode At" << Simulator::Now().GetSeconds()
      << "s, AODV intermediate node " << localNode
      << " send a RREP packet " << packet->GetUid()
      << " for " << toOrigin.GetDestination ()
      << " Ip next hop " << toOrigin.GetNextHop ()
      << " local sending interface " << toOrigin.GetInterface ()
  );
  ...
  if (gratRep)
    {
     ...
     /*BH*/ m_rrepSnt++;
      ...
      NS_LOG_DEBUG ( "RoutingProtocol::SendReplyByIntermediateNode At" << Simulator::Now().GetSeconds()
        << "s, AODV intermediate node " << localNode
        << " send a gratuitious RREP packet " << packetToDst->GetUid()
        << " for " << toOrigin.GetDestination ()
        << " Ip next hop " << toDst.GetNextHop ()
        << " local sending interface " << toDst.GetInterface ()
      );
      packetToDst->Print(std::cout); std::cout << std::endl;
    }
}
...
RoutingProtocol::RecvReply (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
{
  ...
  NS_LOG_DEBUG ("=========> AODV message from " << sender << " packet " << p->GetUid () << " with type: " << 2 << "RREP");
  ...
  if (m_routingTable.LookupRoute (dst, toDst))
    {
      ...
      /*BH*/NS_LOG_DEBUG ("  "<< Simulator::Now().GetSeconds()
    		      << "s. On node " << receiver
    		      << " route for " << dst
    		      << " updated (" << toDst.GetHop()
    		      << " hops -> " << toDst.GetNextHop()
    		      << ")" 
    		    );
    }
  else
