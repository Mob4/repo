...
//  dist1 = dist2 = a->GetDistanceFrom (b) / sqrt(2);
    ...
    int posA_X = m_map->GetRoadXFromVehicle(posA.x);
    int posA_Y = m_map->GetRoadYFromVehicle(posA.y);
	NS_LOG_DEBUG ("Node A: "<< posA << " posA_X = " << posA_X << " posA_Y = " << posA_Y);

	int posB_X = m_map->GetRoadXFromVehicle(posB.x);
    int posB_Y = m_map->GetRoadYFromVehicle(posB.y);
	NS_LOG_DEBUG ("Node B: "<< posB << " posB_X = " << posB_X << " posB_Y = " << posB_Y);


//    dist1 = m_map->GetLosDistance();
//    dist2 = m_map->GetNLostDistance();

    m_map->GetNLOSDistance(posA, posA_X, posA_Y, posB, posB_X, posB_Y, dist1, dist2);
