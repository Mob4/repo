...
{
  double pathloss, shadowing ; //,  dist;
  shadowing = 0.0;
  if (m_visibilityModel == NULL)
    {
      double distance = a->GetDistanceFrom (b);
      //dist = distance;
      pathloss = CalculateLossInLos (a,b,distance);
      if (m_shadowingModel)
        {
          shadowing = m_shadowingModel->GetLosShadowing (a,b);
        }
    }
  ...
  else
    {
      ...
      pathloss = CalculateLossInNlos (a,b,d1,d2);
      if (m_shadowingModel)
        {
          shadowing = m_shadowingModel->GetNLosShadowing (a,b);
        }
    }
  return (txPowerDbm - pathloss + shadowing);
}
