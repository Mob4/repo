...
void
AnimatorScene::setShowMeta(bool show)
{
    m_showMeta = show;
}
...
AnimatorScene::setShowInterfaceTexts(bool showIp, bool showMac)
{
    ...
    {
        for(QVector <AnimLink *>::const_iterator i = LinkManager::getInstance()->getLinks()->begin();
...
{
    NodePositionStatisticsDialog::getInstance()->setVisible(show);
    if(!show)
    {
    ...
    if(NodeMobilityMgr::getInstance()->isNodeListEmpty())
    {
        AnimatorMode::getInstance()->showPopup("No Nodes have been parsed. Did you load the xml trace file?");
        return false;
    }
    NodePositionStatisticsDialog::getInstance()->doShow();
    return true;
}
...
AnimatorScene::showPacketStats(bool show)
{
    if(!show)
    {
        Packetstatisticsdialog::getInstance()->hide();
        return false;
    }
    ...
    Packetstatisticsdialog::getInstance()->doShow();
                       ...
                       QColor * color)
{
    ...
    if (color)
    {
        NodeUpdateManager::getInstance()->addNodeUpdate(0, aNode, color->red(), color->green(), color->blue(), description, true);
    }
    else
    {
        NodeUpdateManager::getInstance()->addNodeUpdate(0, aNode, 255, 0, 0, description, true);
    }
}
...
AnimatorScene::updateNode(qreal updateTime, uint32_t nodeId, uint8_t r, uint8_t g, uint8_t b, QString description, bool visible)
{
    ...
    if(animNode)
    {
        NodeUpdateManager::getInstance()->addNodeUpdate(updateTime, animNode, r, g, b, description, visible);
    }
}
...
AnimatorScene::showPacket(AnimPacket * packet,double currentTime)
{
    ...
    packet->m_graphicsItem->show();



    if (!m_showMeta || ((packet->getShortMeta() == "")))
    {
        return;
    }
...
{
    for(LinkManager::AnimLinkVector_t::const_iterator i = LinkManager::getInstance()->getLinks()->begin();
        i != LinkManager::getInstance()->getLinks()->end() ;
        ++i)
    {
        (*i)->setVisible(show);
